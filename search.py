...
class SearchEngine:
    ...
    def search(self, query: Optional[str] = None, top_n: int = 50) -> List[Tuple[str, float]]:
        """Retourne les séries classées par similarité TF-IDF."""
    ...
    def has_all_terms(self, series_name: str, token_indices: List[int]) -> bool:
        """Retourne True si la série contient tous les tokens donnés."""
    ...
    def keyword_scores(self, tokens: List[str]) -> Dict[str, float]:
        """Calcule un score strict basé sur la présence réelle des tokens."""
        if not tokens:
            return {}
...
def keyword_search(query: str, top_n: int = 50) -> List[Tuple[str, float]]:
    """
    Recherche SQL stricte : ne conservent que les séries contenant tous les mots
    (termes de plus de 2 caractères) présentés dans la requête.
    """
    from collections import defaultdict
